...
class TrainModel:
    ...
    def run_training(self):
        ...
        sampler_training = DistributedSampler(train_dataset)
        ...
        if self.parameters["load_existing_model"]:
            checkpoint = torch.load(os.path.join(get_fsct_path("model_mix_6_3_500"), self.parameters["model_filename_load"]))
        ...
        if self.parameters["load_existing_model"]:
            best_val_acc=torch.load("./model_mix_6_3_500/best_model.pth")["val_acc"]
        else:
            best_val_acc=0
                ...
                if self.parameters["perform_validation_during_training"]:
                    ...
                    with torch.no_grad():
                        for data in self.validation_loader:
                            ...
                            #     save_checkpoint(model_mix_6_3_500, optimizer, epoch, val_epoch_acc,f"/data/LIDAR_RA/FSCT/model_mix_6_3_500/ckpt_{epoch}.pth")
                        ...
                        save_checkpoint(model, optimizer, epoch, val_epoch_acc,
                                        f"/data/FSCT/model_mix_6_3_500/ckpt_{epoch}.pth")
                        ...
                        if improved:
                            save_checkpoint(model, optimizer, epoch, val_epoch_acc,"/data/FSCT/model_mix_6_3_500/best_model.pth")
