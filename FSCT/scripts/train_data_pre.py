...
class TrainModel:
    ...
    @staticmethod
    def threaded_boxes(point_cloud, box_size, min_points_per_box, max_points_per_box, path, id_offset, point_divisions,device):
        ...
        while i < pds:
            ...
            if box.shape[0] > min_points_per_box:
                if box.shape[0] > max_points_per_box:
                    # Move points to GPU
                    pts_torch = torch.from_numpy(box).float().cuda()
                    # FPS ratio to reduce to max_points_per_box
                    ratio = float(max_points_per_box) / float(pts_torch.size(0))
                    idx = fps(pts_torch, ratio=ratio)
                    # Sampled points back to CPU
                    box = pts_torch[idx].cpu().numpy().astype('float32')
                else:
    ...
    def preprocess_point_cloud(self, point_cloud, sample_dir):
        ...
        # Shift point cloud to origin (FSCT original)
        ...
        # Split box_centres among processes
        num_processes = self.parameters["num_cpu_cores_preprocessing"]
        point_divisions = [[] for _ in range(num_processes)]
        points_to_assign = box_centres.copy()
        ...
        while points_to_assign.shape[0] > 0:
            for i in range(num_processes):
                ...
                if points_to_assign.shape[0] == 0:
                    break
        point_divisions = [np.vstack(div) for div in point_divisions]  # convert lists to arrays

        # Compute starting id_offset
        id_offset_base = 0
        training_data_list = glob.glob(sample_dir + "*.npy")
        if len(training_data_list) > 0:
            id_offset_base = np.max([int(os.path.basename(i).split(".")[0]) for i in training_data_list]) + 1

        # Multiprocessing start method
        mp.set_start_method('spawn', force=True)

        processes = []
        for i, divisions in enumerate(point_divisions):
            # Compute unique id_offset for this process
            id_offset = id_offset_base + sum(len(div) for div in point_divisions[:i])
            # Optional: assign GPU per process (round-robin)
            ...
            p = mp.Process(
                ...
                args=(
                    ...
                    id_offset,
                    divisions,
                    device,  # if threaded_boxes accepts device
                )
            )
            processes.append(p)
            p.start()
    ...
    def update_log(self, epoch, epoch_loss, epoch_acc, val_epoch_loss, val_epoch_acc):
        ...
        try:
            np.savetxt(os.path.join(get_fsct_path("model_mix_6_3_500"), "training_history.csv"), self.training_history)
        except PermissionError:
            print("training_history not saved this epoch, please close training_history.csv to enable saving.")
            try:
                np.savetxt(
                    os.path.join(get_fsct_path("model_mix_6_3_500"), "training_history_permission_error_backup.csv"),
    ...
    def run_training(self):
        ...
        if self.parameters["load_existing_model"]:
            print("Loading existing model_mix_6_3_500...")
            try:
                model.load_state_dict(
                    torch.load(os.path.join(get_fsct_path("model_mix_6_3_500"), self.parameters["model_filename"])),
                    strict=False,
                )

            except FileNotFoundError:
                print("File not found, creating new model_mix_6_3_500...")
                torch.save(
                    model.state_dict(),
                    os.path.join(get_fsct_path("model_mix_6_3_500"), self.parameters["model_filename"]),
                )

            try:
                self.training_history = np.loadtxt(os.path.join(get_fsct_path("model_mix_6_3_500"), "training_history.csv"))
...
if __name__ == "__main__":
    parameters = dict(
        ...
        generate_point_cloud_vis=0,  # Useful for visually checking how well the model_mix_6_3_500 is learning. Saves a set of samples called "latest_prediction.las" in the "FSCT/data/"" directory. Samples have label and prediction values.
