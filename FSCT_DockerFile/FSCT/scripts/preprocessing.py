...
class Preprocessing:
    ...
    def preprocess_point_cloud(self):
        ...
        # Convert divisions to arrays for multiprocessing
        point_divisions = [np.vstack(div) for div in point_divisions if len(div) > 0]
        ...
        processes = []
        prev_id_offset = 0
        for thread in range(len(point_divisions)):
            id_offset = sum(len(point_divisions[t]) for t in range(thread))
            prev_id_offset = id_offset

            # Optional: assign GPU per process (round-robin)
            if torch.cuda.is_available():
                device = f"cuda:{thread % torch.cuda.device_count()}"
            else:
                device = "cpu"
            ...
            p = mp.Process(
                ...
                args=(
                    ...
                    device,  # optional argument for GPU device
                ),
            )
            processes.append(p)
            p.start()
